...
std::vector<CL_String> CL_CSSParserBorderImage::get_names()
{
...
void CL_CSSParserBorderImage::parse(CL_CSSBoxProperties &properties, const CL_String &name, const std::vector<CL_CSSToken> &tokens)
{
	...
	do
	{
		...
		else if (!slice_specified && parse_slice(border_image_slice, pos, tokens))
		{
			...
			size_t next_pos = pos;
			CL_CSSToken token = next_token(next_pos, tokens);
...
bool CL_CSSParserBorderImage::parse_source(CL_CSSBoxBorderImageSource &border_image_source, size_t &parse_pos, const std::vector<CL_CSSToken> &tokens)
{
	...
	if (token.type == CL_CSSToken::type_ident && pos == tokens.size())
	{
		if (equals(token.value, "none"))
			border_image_source.type = CL_CSSBoxBorderImageSource::type_none;
		else
			return false;
	}
	else if (token.type == CL_CSSToken::type_uri && pos == tokens.size())
	{
		border_image_source.type = CL_CSSBoxBorderImageSource::type_image;
...
bool CL_CSSParserBorderImage::parse_slice(CL_CSSBoxBorderImageSlice &border_image_slice, size_t &parse_pos, const std::vector<CL_CSSToken> &tokens)
{
	size_t pos = parse_pos;
	size_t last_pos = pos;
	CL_CSSToken token = next_token(pos, tokens);
	...
	border_image_slice.type = CL_CSSBoxBorderImageSlice::type_values;
	if (num_lengths == 1)
	{
		for (int i = 1; i < 4; i++)
		{
...
bool CL_CSSParserBorderImage::parse_width(CL_CSSBoxBorderImageWidth &border_image_width, size_t &parse_pos, const std::vector<CL_CSSToken> &tokens)
{
	size_t pos = parse_pos;
	size_t last_pos = pos;
	CL_CSSToken token = next_token(pos, tokens);
	...
	border_image_width.type = CL_CSSBoxBorderImageWidth::type_values;
	if (num_lengths == 1)
	{
		for (int i = 1; i < 4; i++)
		{
...
bool CL_CSSParserBorderImage::parse_outset(CL_CSSBoxBorderImageOutset &border_image_outset, size_t &parse_pos, const std::vector<CL_CSSToken> &tokens)
{
	size_t pos = parse_pos;
	size_t last_pos = pos;
	CL_CSSToken token = next_token(pos, tokens);
	...
	int num_lengths;
	CL_CSSBoxBorderImageOutset::ValueType value_types[4];
	CL_CSSBoxLength lengths[4];
	float numbers[4] = { 0, 0, 0, 0 };
	for (num_lengths = 0; num_lengths < 4; num_lengths++)
	{
		if (is_length(token))
		{
			value_types[num_lengths] = CL_CSSBoxBorderImageOutset::value_type_length;
			if (!parse_length(token, lengths[num_lengths]))
				return false;
		}
		else if (token.type == CL_CSSToken::type_number)
		{
			value_types[num_lengths] = CL_CSSBoxBorderImageOutset::value_type_number;
			numbers[num_lengths] = CL_StringHelp::text_to_float(token.value);
		}
		else
		{
	...
	border_image_outset.type = CL_CSSBoxBorderImageOutset::type_values;
	if (num_lengths == 1)
	{
		for (int i = 1; i < 4; i++)
		{
...
bool CL_CSSParserBorderImage::parse_repeat(CL_CSSBoxBorderImageRepeat &border_image_repeat, size_t &parse_pos, const std::vector<CL_CSSToken> &tokens)
{
	size_t pos = parse_pos;
	CL_CSSToken token = next_token(pos, tokens);

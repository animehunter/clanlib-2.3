...
std::vector<CL_String> CL_CSSParserBorderImageOutset::get_names()
{
...
void CL_CSSParserBorderImageOutset::parse(CL_CSSBoxProperties &properties, const CL_String &name, const std::vector<CL_CSSToken> &tokens)
{
	size_t pos = 0;
	...
	else
	{
		...
		properties.border_image_outset.type = CL_CSSBoxBorderImageOutset::type_values;
		if (num_lengths == 1)
		{
			for (int i = 1; i < 4; i++)
			{

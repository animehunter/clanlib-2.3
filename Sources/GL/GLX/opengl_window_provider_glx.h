...
{
public:
	typedef XVisualInfo* (GL_GLFUNC *ptr_glXChooseVisual)(Display *dpy, int screen, int *attrib_list);
	typedef void (GL_GLFUNC *ptr_glXCopyContext)(Display *dpy, GLXContext src, GLXContext dst, unsigned long mask);
	typedef GLXContext (GL_GLFUNC *ptr_glXCreateContext)(Display *dpy, XVisualInfo *vis, GLXContext share_list, Bool direct);
	typedef GLXPixmap (GL_GLFUNC *ptr_glXCreateGLXPixmap)(Display *dpy, XVisualInfo *vis, Pixmap pixmap);
	typedef void (GL_GLFUNC *ptr_glXDestroyContext)(Display *dpy, GLXContext ctx);
	typedef void (GL_GLFUNC *ptr_glXDestroyGLXPixmap)(Display *dpy, GLXPixmap pix);
	typedef int (GL_GLFUNC *ptr_glXGetConfig)(Display *dpy, XVisualInfo *vis, int attrib, int *value);
	typedef GLXContext (GL_GLFUNC *ptr_glXGetCurrentContext)(void);
	typedef GLXDrawable (GL_GLFUNC *ptr_glXGetCurrentDrawable)(void);
	typedef Bool (GL_GLFUNC *ptr_glXIsDirect)(Display *dpy, GLXContext ctx);
	typedef Bool (GL_GLFUNC *ptr_glXMakeCurrent)(Display *dpy, GLXDrawable drawable, GLXContext ctx);
	typedef Bool (GL_GLFUNC *ptr_glXQueryExtension)(Display *dpy, int *error_base, int *event_base);
	typedef Bool (GL_GLFUNC *ptr_glXQueryVersion)(Display *dpy, int *major, int *minor);
	typedef void (GL_GLFUNC *ptr_glXSwapBuffers)(Display *dpy, GLXDrawable drawable);
	typedef void (GL_GLFUNC *ptr_glXUseXFont)(Font font, int first, int count, int list_base);
	typedef void (GL_GLFUNC *ptr_glXWaitGL)(void);
	typedef void (GL_GLFUNC *ptr_glXWaitX)(void);
	typedef const char *(GL_GLFUNC *ptr_glXGetClientString)(Display *dpy, int name);
	typedef const char *(GL_GLFUNC *ptr_glXQueryServerString)(Display *dpy, int screen, int name);
	typedef const char *(GL_GLFUNC *ptr_glXQueryExtensionsString)(Display *dpy, int screen);
	typedef Display *(GL_GLFUNC *ptr_glXGetCurrentDisplay)(void);
	typedef GLXFBConfig *(GL_GLFUNC *ptr_glXChooseFBConfig)(Display *dpy, int screen, const int *attrib_list, int *nelements);
	typedef GLXContext (GL_GLFUNC *ptr_glXCreateNewContext)(Display *dpy, GLXFBConfig config, int render_type, GLXContext share_list, Bool direct);
	typedef GLXPbuffer (GL_GLFUNC *ptr_glXCreatePbuffer)(Display *dpy, GLXFBConfig config, const int *attrib_list);
	typedef GLXPixmap (GL_GLFUNC *ptr_glXCreatePixmap)(Display *dpy, GLXFBConfig config, Pixmap pixmap, const int *attrib_list);
	typedef GLXWindow (GL_GLFUNC *ptr_glXCreateWindow)(Display *dpy, GLXFBConfig config, Window win, const int *attrib_list);
	typedef void (GL_GLFUNC *ptr_glXDestroyPbuffer)(Display *dpy, GLXPbuffer pbuf);
	typedef void (GL_GLFUNC *ptr_glXDestroyPixmap)(Display *dpy, GLXPixmap pixmap);
	typedef void (GL_GLFUNC *ptr_glXDestroyWindow)(Display *dpy, GLXWindow win);
	typedef GLXDrawable (GL_GLFUNC *ptr_glXGetCurrentReadDrawable)(void);
	typedef int (GL_GLFUNC *ptr_glXGetFBConfigAttrib)(Display *dpy, GLXFBConfig config, int attribute, int *value);
	typedef GLXFBConfig *(GL_GLFUNC *ptr_glXGetFBConfigs)(Display *dpy, int screen, int *nelements);
	typedef void (GL_GLFUNC *ptr_glXGetSelectedEvent)(Display *dpy, GLXDrawable draw, unsigned long *event_mask);
	typedef XVisualInfo *(GL_GLFUNC *ptr_glXGetVisualFromFBConfig)(Display *dpy, GLXFBConfig config);
	typedef Bool (GL_GLFUNC *ptr_glXMakeContextCurrent)(Display *display, GLXDrawable draw, GLXDrawable read, GLXContext ctx);
	typedef int (GL_GLFUNC *ptr_glXQueryContext)(Display *dpy, GLXContext ctx, int attribute, int *value);
	typedef void (GL_GLFUNC *ptr_glXQueryDrawable)(Display *dpy, GLXDrawable draw, int attribute, unsigned int *value);
	typedef void (GL_GLFUNC *ptr_glXSelectEvent)(Display *dpy, GLXDrawable draw, unsigned long event_mask);

	typedef __GLXextFuncPtr (GL_GLFUNC *ptr_glXGetProcAddress) (const GLubyte *);
	typedef void (*(GL_GLFUNC *ptr_glXGetProcAddressARB)(const GLubyte *procName))(void);
